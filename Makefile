...
setup-test-e2e: kind kubectl ## Set up a Kind cluster for e2e tests if it does not exist
	@case "$$($(KIND) get clusters)" in \
		...
		*) \
			...
			$(KIND) create cluster --name $(KIND_CLUSTER) --wait 4m || { \
				echo "The cluster did not come up; the runner is probably loaded. Retrying once."; \
				$(KIND) delete cluster --name $(KIND_CLUSTER) >/dev/null 2>&1 || true; \
				$(KIND) create cluster --name $(KIND_CLUSTER) --wait 6m; \
			} ;; \
	esac

.PHONY: test-e2e
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests in Kind against Moto, then tear everything down.
	@status=0; \
	PATH="$(LOCALBIN):$$PATH" KIND="$(KIND)" KIND_CLUSTER="$(KIND_CLUSTER)" MOTO_IMAGE="$(MOTO_IMAGE)" \
