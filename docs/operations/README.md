...
```sh
NS=aws-subnet-operator-system

kubectl -n $NS get pods -l app.kubernetes.io/name=aws-subnet-operator
kubectl -n $NS logs deployment/aws-subnet-operator-controller-manager -c manager --tail=100
kubectl get networkscopes                      # VPCs / Subnets / Unmanaged / Ready / Last sync
kubectl get networkscope <scope> -o yaml | yq '.status.targets'   # per account/region
```
