...
- `status.targets[].error` for that account/region carries the AWS error verbatim, while
  `vpcs`, `subnets` and `lastSyncTime` keep their **last good** values — "so a flapping
...
- It does not zero the numbers, and it does not mark the whole scope failed — the other
  targets sync normally and `status.vpcs` / `status.subnets` still count everything present.
...
**What you see.** The objects disappear from `kubectl get subnets`, the counts in the scope
...
| `VPCNotFound` | the VPC has not been discovered (wrong ID, wrong account, or the tag selector excludes it) | check `kubectl get vpcs`, the selector, and that the target is healthy |
...
`aws.hypersurgery/scope` label of another scope, skips the object and logs
