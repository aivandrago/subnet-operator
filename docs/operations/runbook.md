...
```promql
absent(up{job="<release>-aws-subnet-operator-metrics", namespace="NS"} == 1)
```
...
```sh
kubectl -n $NS get pods -l app.kubernetes.io/name=aws-subnet-operator
kubectl -n $NS describe pods -l app.kubernetes.io/name=aws-subnet-operator | tail -30
kubectl -n $NS logs deployment/<release>-aws-subnet-operator -c manager --previous --tail=100
```
...
`TargetStatus.Error != ""`), so it is exactly as truthful as `kubectl get networkscope <scope> -o
...
kubectl get networkscope <scope> -o jsonpath='{.status.conditions[?(@.type=="Ready")].message}'
kubectl get networkscope <scope> -o jsonpath='{range .status.targets[*]}{.account}/{.region}{"\t"}{.error}{"\n"}{end}'
kubectl describe networkscope <scope>            # TargetThrottled events carry the next attempt time
```
...
```sh
kubectl get networkscope <scope> -o jsonpath='{range .status.targets[*]}{.account}/{.region}{"\t"}{.unmanagedIDs}{"\n"}{end}'
```
...
```sh
kubectl describe subnetclaim -n <namespace> <name>     # Ready condition: reason and message
kubectl get subnetclaim -n <namespace> <name> -o jsonpath='{.status.allocations}'
```
...
```sh
kubectl describe resourceimport -n <namespace> <name>   # status.error has the AWS message
```
...
`aws.hypersurgery/reason` annotation says which rule won, and the
`aws.hypersurgery/created-by` annotation should be the operator's own service account — an
