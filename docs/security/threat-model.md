...
|---|---|---|
| Operator ServiceAccount | Cluster-wide get/list/watch on the `network.hypersurgery.dev` CRDs and on namespaces (labels, for `namespaceSelector`), create on `resourceimports`, full control of `networks`/`subnets` and all status subresources, `list` (nothing else) on the user-written kinds of the removed `aws.hypersurgery` group, for the guard against objects 0.8 never migrated, create/patch on `events.k8s.io` Events; Secrets **only** in its own namespace and the namespaces listed in `rbac.credentialSecretNamespaces` | `charts/subnet-operator/templates/rbac.yaml`, kubebuilder markers in `internal/controller/*.go` |
...
`autoimport.go`, `internal/policy`, `internal/webhook/v1alpha1` (now `v1beta1`), `internal/cloud/aws/events`,
