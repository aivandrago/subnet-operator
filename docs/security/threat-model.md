...
|---|---|---|
| Operator ServiceAccount | Cluster-wide get/list/watch on the `network.hypersurgery.dev` CRDs and on namespaces (labels, for `namespaceSelector`), create on `resourceimports` (and, for the migration from `aws.hypersurgery`, on scopes, claims and exports), full control of `networks`/`subnets` and all status subresources; in 0.8 also get/list/watch/update on the old group's user-written kinds and delete on its `vpcs`/`subnets`, create/patch on `events.k8s.io` Events; Secrets **only** in its own namespace and the namespaces listed in `rbac.credentialSecretNamespaces` | `charts/subnet-operator/templates/rbac.yaml`, kubebuilder markers in `internal/controller/*.go` |
...
| R | The operator's audit line names whoever `spec.requestedBy` says, and that field is free text. | Since #70 the line also carries `created_by`: the user the API server authenticated, which the mutating webhook writes into the `network.hypersurgery.dev/created-by` annotation on CREATE (overwriting any value sent), and which the validating webhook requires to match the requesting user on CREATE and refuses to change on UPDATE (`stampCreatedBy`, `validateCreatedByOnCreate`, `validateCreatedByOnUpdate` in `internal/webhook/v1beta1/common.go`). Imports written by the auto-import policy carry the operator's own identity. One exception, for the migration from `aws.hypersurgery` (#42): an object the operator's own service account creates with the `network.hypersurgery.dev/migrated-from` annotation keeps the created-by value it carries, copied from the old object, whose webhook guarded it the same way; such a copy also skips the content checks the old webhook already made (`keepsCopiedCreator`, `createdByMigration`; tests in `tenancy_webhook_test.go`). Anybody else setting the marker gets their own name. In 0.8 the old group's webhook still stamps and guards `aws.hypersurgery/created-by` (`internal/webhook/v1alpha1`). `principal` stays the free-text attribution and is documented as such ([docs/audit.md](../audit.md#what-created_by-means)). | With `webhook.enabled=false` nothing guards the annotation, so the audit line says `created_by: unknown`; the Kubernetes audit log is then the only record of who created an object. Whoever holds the operator's service account token can create objects naming any creator — they can also do everything the operator can. An old object created while the old group's webhooks were off carries whatever annotation it was given, and the migration copies it. |
...
`autoimport.go`, `internal/policy`, `internal/webhook/v1alpha1` (now `v1beta1`), `internal/events`,
...
- A `NetworkScope` migrated from `aws.hypersurgery/v1alpha1` without a `namespaceSelector` gets
  `{}` and can still be used from every namespace (warned about at apply time and by an Event).
...
5. **Kubernetes RBAC and tenancy.** `charts/subnet-operator/templates/rbac.yaml` and the
   ...
   or be created with a `created-by` annotation that is not its creator — including through the
   migration's exception for the operator's own copies (`internal/migration`,
   `keepsCopiedCreator`)?
