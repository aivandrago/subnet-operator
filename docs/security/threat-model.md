...
[Findings](#findings-of-the-10-review), and has been kept current since: it describes the code
of 1.0 (the `network.hypersurgery.dev/v1` API, the provider registry with AWS as its one
provider, the conversion webhook). A change that
...
| **Integrity of the inventory** (`Network`, `Subnet` objects, scope status, metrics) | Alerts, capacity planning and CIDR allocation (`SubnetClaim`) trust it. A wrong free-IP count or a hidden overlap leads to a wrong subnet. |
...
|---|---|---|
| Operator ServiceAccount | Cluster-wide get/list/watch on the `network.hypersurgery.dev` CRDs and on namespaces (labels, for `namespaceSelector`), create on `resourceimports`, full control of `networks`/`subnets` and all status subresources, `list` (nothing else) on the user-written kinds of the removed `aws.hypersurgery` group, for the guard against objects 0.8 never migrated, create/patch on `events.k8s.io` Events; on its own six CRDs, by name and nothing else of `apiextensions.k8s.io`, `get`/`patch` (their conversion) and `get`/`update` on their status (trimming `storedVersions` after an upgrade); Secrets **only** in its own namespace and the namespaces listed in `rbac.credentialSecretNamespaces` | `charts/subnet-operator/templates/rbac.yaml`; `config/rbac/role.yaml` (kustomize and the OLM bundle), generated from the kubebuilder markers in `internal/controller/*.go`, `internal/crdversions` and `internal/migration` |
...
| Dashboard pod | Nothing: `automountServiceAccountToken: false`, no RBAC. Acts only with the viewer's bearer token | `charts/subnet-operator/templates/dashboard.yaml`, `internal/dashboard/proxy.go` |
...
|---|---|---|---|
| E | A user who can create a `SubnetClaim`/`ResourceImport` in *any* namespace makes the operator write in *any* account a scope has a write role for — the operator acts as a confused deputy across tenants. | `NetworkScope.spec.namespaceSelector` (#69) names the namespaces that may refer to the scope. The validating webhooks refuse a claim or import from any other namespace (`validateNamespace` in `internal/webhook/v1/common.go`); the controllers refuse it in the status with `NamespaceNotAllowed` before reserving a CIDR or calling the provider's `SubnetWriter`/`OwnershipWriter`, for objects created while the webhooks were off (`namespaceRefusal` in `internal/controller/subnetclaim_controller.go`, used by both controllers; tests in `internal/controller/tenancy_test.go`); the auto-import policy only writes into a namespace the scope allows (webhook and `autoImportAllowed` in `internal/controller/autoimport.go`); a selector that does not parse allows no namespace, and neither does an unset one (`AllowsNamespace` in `api/v1/networkscope_types.go`). Writes still need `--enable-writes` **and** a `writeRoleARN`; the webhooks refuse accounts the scope does not cover; no aggregated `edit`/`admin` roles are shipped. | A scope with an **empty** selector (`{}`) allows every namespace; the webhook warns and the scope gets a `NamespacesUnrestricted` Event. The migration from `aws.hypersurgery/v1alpha1` (0.8, and `manager migrate-manifests` since), where an unset selector allowed every namespace, writes exactly that into such scopes so nobody is locked out, and says so: until an administrator narrows it, the old exposure stays. Namespace labels are only as trustworthy as the RBAC on `namespaces`: select by `kubernetes.io/metadata.name`, which the API server sets, or by labels tenants cannot write. Narrowing a selector stops new writes but leaves what existing objects already did in place. |
...
| T | A claim or import points at a resource in another account than it names. | Per-provider format checks: CEL on the scope for 12-digit AWS accounts, and the webhooks and controllers for claims and imports (the AWS provider's `ValidateClaim`, `ValidateImport`, `ClaimRefusal` and `ImportRefusal` in `internal/cloud/aws/validate.go`: 12-digit accounts, `vpc-`/`subnet-` IDs, zones of the region); webhook compares the inventory's account/region with the spec; the role ARN's account must equal the target account (`credentialsFor` in `internal/cloud/aws/discoverer.go`), so a write can only ever happen in the account the object names. | — |
| T | Editing mirrored `Network`/`Subnet` objects to mislead allocation. | They are outputs; the next sync overwrites them. README: give users read-only RBAC on them. | A user with write RBAC on `subnets` can hide a CIDR from the allocator until the next sync; `CreateSubnet` then fails with a conflict and the claim reallocates (`ErrCIDRConflict`). |
| R | The operator's audit line names whoever `spec.requestedBy` says, and that field is free text. | Since #70 the line also carries `created_by`: the user the API server authenticated, which the mutating webhook writes into the `network.hypersurgery.dev/created-by` annotation on CREATE (overwriting any value sent), and which the validating webhook requires to match the requesting user on CREATE and refuses to change on UPDATE (`stampCreatedBy`, `validateCreatedByOnCreate`, `validateCreatedByOnUpdate` in `internal/webhook/v1/common.go`). Imports written by the auto-import policy carry the operator's own identity. There is no exception: 0.8 let the operator's own service account keep a copied value on an object marked `network.hypersurgery.dev/migrated-from`, for its migration from `aws.hypersurgery` (#42); 0.9 migrates nothing and dropped it, so the marker grants nothing, to the operator neither (tests in `tenancy_webhook_test.go`). `principal` stays the free-text attribution and is documented as such ([docs/audit.md](../audit.md#what-created_by-means)). | With `webhook.enabled=false` nothing guards the annotation, so the audit line says `created_by: unknown`; the Kubernetes audit log is then the only record of who created an object. Whoever holds the operator's service account token can create objects naming any creator — they can also do everything the operator can. An object 0.8 migrated from one created while the old group's webhooks were off carries whatever creator it was given. |
...
| I | Tags mirrored into `Subnet`/`Network` status are readable by anyone with read RBAC on them. | Documented in `SECURITY.md` ("put nothing secret in tags"). | By design. |

### B2 — Operator → AWS, read path

| | Threat | Mitigation today | Residual |
|---|---|---|---|
| S / E | A scope author points `roleARN` at a role in another account and reads it. | `credentialsFor` refuses a role whose ARN account differs from the declared account; the webhook refuses it at apply time (`validateAccount` in `internal/cloud/aws/validate.go`, called through the provider's `ValidateScope`); only one account may go without a role. | Anyone who can create a `NetworkScope` can make the operator assume any role that trusts the operator role. That is why `NetworkScope` is an admin object. |
...
| R | A write without a record. | Every allocation, creation, import and policy decision emits an audit line (`internal/audit`, [docs/audit.md](../audit.md)) with the authenticated `created_by`, plus an Event on the object; AWS CloudTrail records the operator's session name. | — |
...
| | Threat | Mitigation today | Residual |
|---|---|---|---|
| S / T | A forged message claims a resource was created by a principal of the attacker's choosing, steering `fromCreator` rules. | The queue policy admits only `events.amazonaws.com` for the one rule (`aws:SourceArn`, `deploy/events/hub-events.cfn.yaml`); EventBridge refuses `aws.*` sources from `PutEvents`, so organization members can forward real EC2 events but not forge them; only `source: aws.ec2` events without `errorCode` are read (`internal/cloud/aws/events/events.go`). | Principals in the hub account with `sqs:SendMessage` through an identity policy bypass the queue policy. Creator attribution is keyed by resource ID only, not by account (`internal/controller/creators.go`). |
| D | A flood of messages. | Parsing is a single `json.Unmarshal` of a message SQS caps in size; unparsable messages are dropped and deleted (`internal/cloud/aws/events/poller.go`); changes are debounced (`--aws-events-debounce`, 10 s) and only targets of existing scopes are synced; the creator cache is bounded (512 entries, 24 h). | A steady forged stream can make every target sync every debounce interval: up to 4 EC2 calls per target per 10 s. |
| I | — | The queue is SSE-encrypted, including the dead-letter queue. | — |

### B5 — kube-apiserver → admission and conversion webhooks

| | Threat | Mitigation today | Residual |
|---|---|---|---|
| S | Someone impersonates the webhook and approves or rewrites objects. | TLS with a CA bundle in the webhook configurations, from cert-manager or a chart-signed certificate (`charts/subnet-operator/templates/webhook.yaml`). cert-manager is the documented setup for production and GitOps (chart README, `values.yaml`, install guide). | **Open: #71** — without cert-manager, the chart-signed certificate is valid for 10 years, never rotated, its key is stored in every Helm release Secret of the namespace, and `helm template` (Argo CD, Flux with post-rendering) signs a new CA on every render. Accepted for installs that follow the recommendation; rotation is tracked in #71. |
| T | A webhook outage lets invalid objects in. | Validating webhooks are `failurePolicy: Fail` by default; mutating ones are `Ignore` but only fill defaults the controllers also apply, and the one thing they write that nothing else does — the created-by annotation — is checked again by the validating webhook; the controllers re-check what decides a write — the account is in the scope, the write role exists, the network is where the claim says — and report it in the status (`internal/webhook/v1/common.go`, package comment). | With `webhook.enabled=false` the only checks are the controllers'. |
...
| S / T | Someone impersonates the conversion webhook, or the operator's own identity is used to point it elsewhere, and rewrites objects as they are converted between v1beta1 and v1. | The API server calls `/convert` over TLS with the CA bundle the operator puts in each CRD: `ca.crt` of the webhook certificate, the same trust as the admission webhooks (`internal/crdversions`). The operator may patch only its own six CRDs, by name. Everything is stored at v1 after the first start of 1.0, so the operator's own reads and writes need no conversion; only clients that still use v1beta1 go through it. | Whoever holds the operator's service account token can patch those six CRDs, their conversion and their schema included. It can already write every status and every `Network` and `Subnet`; the CRD rule adds the specs of what clients read or write at v1beta1, and loosening the CRDs' own validation. `webhook.conversion.enabled=false` keeps the conversion at `None` but not the permission, which the operator needs to set it. |
| D | The conversion webhook is down. | Requests at v1 need no conversion once the storage is migrated, and the operator switches the CRDs to `Webhook` only after that; until then they use the API server's own conversion. | Requests at v1beta1 fail while no operator pod is ready, and after an uninstall until the CRDs are set back to `None` ([upgrades](../operations/upgrades.md#reading-and-writing-v1beta1)). |
...
| E | A forwarded request reaches something other than this API group. | Allowlist: GETs under `/apis/network.hypersurgery.dev/<version>/…`, POST of `resourceimports` in a namespace, and POST of an empty `SelfSubjectReview` without query parameters (to show who is signed in; its body is checked field by field); escaped or non-canonical paths refused; headers rebuilt from an allowlist, so `Impersonate-*` and front-proxy headers never pass (`Allowed`, `checkSelfSubjectReview`, `rewrite`). | The query string of the network.hypersurgery.dev calls is forwarded unchanged (harmless for GET and a create; revisit if methods are added). |
...
| | Threat | Mitigation today | Residual |
|---|---|---|---|
| T | A tampered image, chart or OLM bundle. | Built by `.github/workflows/release.yml` from the tag; image, chart and (from 1.0) the OLM bundle image signed keyless with cosign, the bundle pinning the image by digest; the job verifies its own signature the way users are told to; buildx SBOM and `mode=max` provenance attached; actions pinned by commit SHA; distroless, non-root, read-only root filesystem (`Dockerfile.release`, `values.yaml`). | — |
...
`autoimport.go`, `internal/policy`, `internal/webhook/v1alpha1` (now `v1`), `internal/cloud/aws/events`,
...
- A `NetworkScope` migrated from `aws.hypersurgery/v1alpha1` (by 0.8, or converted with
  `manager migrate-manifests`) without a `namespaceSelector` gets `{}` and can still be used from
  every namespace (warned about at apply time and by an Event).
  Set a real selector on every scope that holds a write role.
- Without cert-manager, the webhook serving certificate is long-lived and never rotated (#71).
- The operator may `get` and `patch` its own six CustomResourceDefinitions, by name, to set
  their conversion ([B5](#b5--kube-apiserver--admission-and-conversion-webhooks)). Whoever holds
  its service account token can therefore redirect their conversion or loosen their schema.
  Kept on purpose (owner decision, 2026-09-25): the files in the chart's `crds/` cannot name
  the release's Service or carry its CA, and rendering the CRDs as chart templates instead
  would make Helm own them, so that `helm uninstall` deletes every object with them. The rule
  names the six CRDs and nothing else of `apiextensions.k8s.io` (`resourceNames` in
  `config/rbac/role.yaml` and the chart's ClusterRole, checked against `crdversions.CRDNames()`
  by a chart test), and the token is the same one that can already write every status and
  every `Network` and `Subnet`. `webhook.conversion.enabled=false` does not remove the rule.
  The CA path with cert-manager is covered by render tests and envtest; an end-to-end run with
  cert-manager is issue #86.
...
1. **The write path end to end.** `SubnetClaim` → `SubnetClaimReconciler.reconcile` →
   `writeTarget` (the provider's write identity, `Provider.Identity(…, Write)`) →
   `Provider.CreateSubnet` → `Discoverer.CreateSubnet` → `credentialsFor`; `ResourceImport` →
   `ResourceImportReconciler.reconcile` → `Provider.WriteOwnership` → `ApplyTags`. Can any input
   make the operator write in an account, or to a resource, other than the one the object names?
   Can a write happen with `--enable-writes` off or without a `writeRoleARN`? (A target that
   carries another provider's identity is refused, not treated as the operator's own:
   `identityOf` in `internal/cloud/aws/provider.go`.)
2. **The auto-import policy as an attack surface.** `internal/controller/autoimport.go`,
   ...
   operator tag?
3. **SQS input handling.** `internal/cloud/aws/events/events.go` and `poller.go`: parsing of untrusted
...
5. **Kubernetes RBAC and tenancy.** `charts/subnet-operator/templates/rbac.yaml` and the
   ...
   or be created with a `created-by` annotation that is not its creator? And does the guard in
   `internal/migration/guard.go` hold the operator back whenever an old-group object 0.8 never
   migrated exists, including when it cannot list them?
6. **IAM.** `deploy/iam/*`: the trust policies, the optional ExternalID, and the resource
   limits of the write role.
7. **Admission and conversion webhooks.** Fail-open/closed behaviour, the self-signed
   certificate (#71), and the operator's right to patch its own CRDs to configure their
   conversion (`internal/crdversions`).
