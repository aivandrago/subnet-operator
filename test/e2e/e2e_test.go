...
`, scopeName, hubAccount, awsRegion, subnet))
		...
		It("serves the dashboard, which reads the cluster as the viewer and never as itself", func() {
			const dashboard = "aws-subnet-operator-dashboard"
			...
			Expect(err).NotTo(HaveOccurred())
			helm := exec.Command(envOr("HELM", "helm"), "template", "aws-subnet-operator", "charts/aws-subnet-operator",
			...
			for _, args := range [][]string{
				...
				{"create", "clusterrole", "dashboard-e2e-reader", "--verb=get,list,watch", "--resource=subnets.aws.hypersurgery"},
			...
			subnets := base + "/apis/aws.hypersurgery/v1alpha1/subnets"
			script := strings.Join([]string{
				// The Service may take a moment to route to the new pod.
				"for i in $(seq 1 30); do curl -sf -o /dev/null " + base + "/dashboard/ && break; sleep 2; done",
				"call() { echo \"== $1\"; shift; curl -s -w '\\ncode=%{http_code}\\n' \"$@\"; }",
