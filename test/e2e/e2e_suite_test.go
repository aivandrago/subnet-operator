...
var (
	// managerImage is the manager image to be built and loaded for testing. It is tagged per
	// Kind cluster because parallel runs on one runner share its Docker daemon: under a fixed
	// tag, a run could load into its cluster the image another run had just built from a
	// different commit.
	managerImage = "example.com/subnet-operator:" + envOr("KIND_CLUSTER", "kind")
...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting subnet-operator e2e test suite\n")
