...
`kubectl get nscope <name> -o yaml` (redact account IDs if you like — the shape matters,
...
2. `make test` runs unit tests and envtest; `make lint` runs golangci-lint with the
   ...
   The e2e suite runs on Linux and on macOS (Apple silicon, Docker under colima). It needs
   Docker, `make`, `curl` and Go; `make` fetches its own Kind, kubectl and Helm into `bin/`.
   Each concurrent run adds a Kind node and a Moto container to the Docker host. Every run is
