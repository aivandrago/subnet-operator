...
type fakeSheets struct {
	...
	rowCount      int64
	protected     bool
	// protectedAs is the description of the existing protected range; protectionDescription
	// when empty.
	protectedAs string
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && strings.HasSuffix(path, "/v4/spreadsheets/sheet-1"):
			...
			if f.existingTitle != "" {
				...
				if f.protected {
					description := f.protectedAs
					if description == "" {
						description = protectionDescription
					}
					sheet.ProtectedRanges = []*sheetsapi.ProtectedRange{{Description: description}}
				}
...
// A sheet exported before the project was renamed carries the old name in its protected range.
// It is protected already; a second range would only be clutter.
func TestSyncRecognisesTheProtectionOfEarlierReleases(t *testing.T) {
	fake := &fakeSheets{existingTitle: "Subnets", rowCount: 2, protected: true,
		protectedAs: "Maintained by aws-subnet-operator - edits are overwritten"}
	if err := newTestService(t, fake).Sync(context.Background(), "sheet-1", "Subnets", table()); err != nil {
		t.Fatal(err)
	}
	for _, rq := range fake.batches[0].Requests {
		if rq.AddProtectedRange != nil {
			t.Error("a sheet protected under the old name got a second protected range")
		}
	}
}
