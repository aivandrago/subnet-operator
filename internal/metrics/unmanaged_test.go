...
func TestSeedingStartsFromWhatWasAlreadyKnown(t *testing.T) {
	...
	t.Cleanup(func() { Forget(scope) })
	seen := func() float64 {
		return testutil.ToFloat64(unmanagedSeen.WithLabelValues(scope, account, region, "subnet"))
	}
	...
	Unmanaged(scope, account, region, "subnet", []string{"subnet-a", "subnet-b", "subnet-c"})
	if got := seen(); got != 1 {
