...
// Resource is the unmanaged VPC or subnet under consideration.
type Resource struct {
	// ID is the vpc-… or subnet-… identifier.
	ID string
	// Account and Region locate it.
	Account, Region string
	// Tags are the tags it already carries.
	Tags map[string]string
	// IsSubnet says whether inheritFromVPC applies.
	IsSubnet bool
	// ParentVPCTags are the tags of the VPC a subnet belongs to; empty for a VPC.
	ParentVPCTags map[string]string
}
...
func Decide(r Resource, creator string, p *awsv1alpha1.AutoImportPolicy) Decision {
	...
	// Then the VPC: a subnet added by hand usually belongs to whoever owns the network.
	if len(tags) == 0 && r.IsSubnet && len(p.InheritFromVPC) > 0 {
		for _, key := range p.InheritFromVPC {
			if v, ok := r.ParentVPCTags[key]; ok && v != "" {
				tags[key] = v
			}
		}
		if len(tags) > 0 {
			reason = "tags inherited from the parent VPC"
		}
	}
	...
	if managedTag == "" {
		managedTag = awsv1alpha1.DefaultManagedTag
	}
	if managedValue == "" {
		managedValue = awsv1alpha1.DefaultManagedValue
	}
...
func matchSkip(r Resource, creator string, rules []awsv1alpha1.SkipRule) (awsv1alpha1.SkipRule, bool) {
	...
	return awsv1alpha1.SkipRule{}, false
}

func skipReason(rule awsv1alpha1.SkipRule, creator string) string {
	switch {
