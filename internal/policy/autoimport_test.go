...
func subnet(tags, parent map[string]string) Resource {
	return Resource{ID: "subnet-0a1b2c3d", Account: "222222222222", Region: "eu-central-1",
		Tags: tags, IsSubnet: true, ParentNetworkTags: parent}
}
...
func TestDecide(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "inherited from the network when no creator rule matches",
		...
		{
			// The creator rule says team-payments, but somebody already tagged the subnet:
			// importing must add the managed tag, not reassign the resource.
			name:     "a rule never overwrites a value the resource already carries",
			resource: subnet(map[string]string{"hs/owner": "team-data"}, nil), creator: payments,
			policy: fullPolicy(), want: VerdictImport,
			tags: map[string]string{"hs/managed": "true"},
		},
		{
			name:     "an empty value is missing, not set",
			resource: subnet(map[string]string{"hs/owner": ""}, map[string]string{"hs/owner": "team-web"}),
			creator:  console, policy: fullPolicy(), want: VerdictImport,
			tags: map[string]string{"hs/owner": "team-web", "hs/managed": "true"},
		},
	}
...
func TestDecideRequiredTags(t *testing.T) {
	...
	// With the environment inherited from the network it resolves... except that inheritance only
