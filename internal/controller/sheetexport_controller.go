...
// +kubebuilder:rbac:groups=network.hypersurgery.dev,resources=sheetexports/status,verbs=get;update;patch
// The credentials live next to the operator, so the Secret permission stays namespaced:
// a cluster-wide one would let the operator read every Secret in the cluster.
// +kubebuilder:rbac:groups="",namespace=subnet-operator-system,resources=secrets,verbs=get;list;watch
...
func (r *SheetExportReconciler) export(ctx context.Context, export *networkv1beta1.SheetExport) (int, error) {
	...
	for _, v := range networks.Items {
		networkNames[v.Spec.ID] = v.Status.Name
	}
	...
	table := sheets.BuildTable(subnets.Items, networkNames, syncedAt, export.Spec.ExtraTagColumns)
