...
var _ = Describe("SheetExport Controller", func() {
	...
	getExport := func() *awsv1alpha1.SheetExport {
		GinkgoHelper()
		e := &awsv1alpha1.SheetExport{}
	...
	BeforeEach(func() {
		...
		scope := &awsv1alpha1.NetworkScope{
			...
			Spec: awsv1alpha1.NetworkScopeSpec{
				Accounts: []awsv1alpha1.AccountSpec{{ID: exportAccount}},
		...
		scope.Status.Targets = []awsv1alpha1.TargetStatus{{Account: exportAccount, Region: exportRegion, LastSyncTime: &synced}}
		Expect(k8sClient.Status().Update(ctx, scope)).To(Succeed())

		labels := map[string]string{
			awsv1alpha1.LabelScope: scopeName, awsv1alpha1.LabelAccount: exportAccount,
			awsv1alpha1.LabelRegion: exportRegion, awsv1alpha1.LabelVPC: "vpc-e1",
		}
		vpc := &awsv1alpha1.VPC{
			ObjectMeta: metav1.ObjectMeta{Name: "vpc-e1-" + exportName, Labels: labels},
			Spec:       awsv1alpha1.VPCSpec{VPCID: "vpc-e1", Account: exportAccount, Region: exportRegion},
		}
		...
		for i, cidr := range []string{"10.0.2.0/24", "10.0.1.0/24"} {
			sn := &awsv1alpha1.Subnet{
				...
				Spec: awsv1alpha1.SubnetSpec{SubnetID: fmt.Sprintf("subnet-e%d", i), VPCID: "vpc-e1",
					Account: exportAccount, Region: exportRegion},
			}
			Expect(k8sClient.Create(ctx, sn)).To(Succeed())
			sn.Status = awsv1alpha1.SubnetStatus{CIDRBlock: cidr, TotalIPs: 251, AvailableIPs: 51,
				UtilizationPercent: 79, Owner: "team-a", Tags: map[string]string{"cost-center": "cc-42"}}
		...
		Expect(k8sClient.Create(ctx, &awsv1alpha1.SheetExport{
			ObjectMeta: metav1.ObjectMeta{Name: exportName},
			Spec: awsv1alpha1.SheetExportSpec{
				ScopeRef: scopeName, SpreadsheetID: "sheet-1", SheetName: "Subnets",
				CredentialsSecretRef: awsv1alpha1.SecretKeyRef{Name: secretName, Namespace: "default", Key: "credentials.json"},
	...
	AfterEach(func() {
		...
		Expect(k8sClient.Delete(ctx, &awsv1alpha1.SheetExport{ObjectMeta: metav1.ObjectMeta{Name: exportName}})).To(Succeed())
	...
	It("writes the scope's subnets into the sheet", func() {
		...
		Expect(syncer.table.Rows[0][6]).To(Equal("10.0.1.0/24"), "rows are sorted by CIDR")
		Expect(syncer.table.Rows[0][3]).To(Equal("prod"), "the VPC name is resolved")
		Expect(syncer.table.Rows[0][17]).To(Equal("2026-09-22 08:05"), "the target's last sync time")
