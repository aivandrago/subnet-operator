...
var _ = Describe("NetworkScope at the documented capacity", Label("scale"), Ordered, func() {
	...
	BeforeAll(func() {
		...
		accounts := make([]awsv1alpha1.AccountSpec, 0, shape.accounts)
		...
		for a := range shape.accounts {
			...
			accounts = append(accounts, awsv1alpha1.AccountSpec{ID: id,
				RoleARN: fmt.Sprintf("arn:aws:iam::%s:role/subnet-inventory-read", id)})
		}
		Expect(k8sClient.Create(ctx, &awsv1alpha1.NetworkScope{
			...
			Spec: awsv1alpha1.NetworkScopeSpec{
				...
				VPCTagSelector:     map[string]string{"hs/owner": ""},
	...
	It("mirrors the whole organization on the first sync", func() {
		...
		scope := getScope()
		Expect(scope.Status.VPCs).To(Equal(int32(nVPCs)))
	...
	It("keeps syncing the healthy targets while a tenth of them is throttled", func() {
		...
		Expect(scope.Status.VPCs).To(Equal(int32(nVPCs)), "throttled targets keep their objects")
