...
var _ = Describe("Namespaces allowed to use a NetworkScope", func() {
	...
	createScope := func(selector *metav1.LabelSelector, policy *awsv1alpha1.AutoImportPolicy) {
		GinkgoHelper()
		Expect(k8sClient.Create(ctx, &awsv1alpha1.NetworkScope{
			...
			Spec: awsv1alpha1.NetworkScopeSpec{
				Accounts:          []awsv1alpha1.AccountSpec{{ID: tenancyAccount}},
	...
	AfterEach(func() {
		for _, ns := range []string{allowedNS, otherNS} {
			Expect(k8sClient.DeleteAllOf(ctx, &awsv1alpha1.SubnetClaim{}, client.InNamespace(ns))).To(Succeed())
			Expect(k8sClient.DeleteAllOf(ctx, &awsv1alpha1.ResourceImport{}, client.InNamespace(ns))).To(Succeed())
		}
	...
	Describe("a SubnetClaim", func() {
		...
		createVPC := func() {
			...
			vpc := &awsv1alpha1.VPC{
				ObjectMeta: metav1.ObjectMeta{Name: vpcID,
					Labels: map[string]string{awsv1alpha1.LabelScope: scopeName, awsv1alpha1.LabelVPC: vpcID}},
				Spec: awsv1alpha1.VPCSpec{VPCID: vpcID, Account: tenancyAccount, Region: tenancyRegion},
			}
		...
		createClaim := func(namespace string, zones ...string) {
			...
			Expect(k8sClient.Create(ctx, &awsv1alpha1.SubnetClaim{
				...
				Spec: awsv1alpha1.SubnetClaimSpec{
					ScopeRef: scopeName, Account: tenancyAccount, Region: tenancyRegion, VPCID: vpcID,
					PrefixLength: 24, AvailabilityZones: zones,
					Mode: awsv1alpha1.ClaimModeCreate, Owner: "team-payments",
				},
			})).To(Succeed())
		}
		reconcileClaim := func(namespace string) *awsv1alpha1.SubnetClaim {
			GinkgoHelper()
		...
		It("keeps what a claim has but writes nothing more once its namespace is no longer allowed", func() {
			...
			claim.Spec.AvailabilityZones = append(claim.Spec.AvailabilityZones, tenancyRegion+"b")
	...
	Describe("a ResourceImport", func() {
		...
		reconcileImport := func(namespace string) *awsv1alpha1.ResourceImport {
			GinkgoHelper()
		...
		It("keeps working for every namespace when the scope has no selector", func() {
			createScope(nil, nil)
			createImport(otherNS)
	...
	Describe("a NetworkScope", func() {
		...
		importsIn := func(namespace string) []awsv1alpha1.ResourceImport {
			GinkgoHelper()
		...
		policyIn := func(namespace string) *awsv1alpha1.AutoImportPolicy {
			return &awsv1alpha1.AutoImportPolicy{
				Mode: awsv1alpha1.AutoImportApply, Namespace: namespace,
				AccountDefaults: []awsv1alpha1.AccountDefault{
		...
		BeforeEach(func() {
			...
			reconciler = &NetworkScopeReconciler{
				...
				Identity: "system:serviceaccount:aws-subnet-operator-system:aws-subnet-operator",
			}
		})

		It("warns once per spec change that it has no namespaceSelector", func() {
			createScope(nil, nil)

			reconcileScope()
			haveEvent(recorder, "Warning "+EventNamespacesUnrestricted, "has no spec.namespaceSelector")
		...
		It("does not warn about a scope that has a selector", func() {
			createScope(paymentsOnly(), nil)
		...
		It("writes imports into an allowed namespace as the operator", func() {
			...
			Expect(imports[0].Annotations).To(HaveKeyWithValue(awsv1alpha1.AnnotationCreatedBy, reconciler.Identity))
