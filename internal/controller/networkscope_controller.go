...
func (r *NetworkScopeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	networks, err := r.updateOverlaps(ctx, scope.Name)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *NetworkScopeReconciler) updateOverlaps(ctx context.Context, scope string) ([]networkv1beta1.Network, error) {
	...
	for i := range networks {
		...
		for j := range networks {
			if i != j && inventory.Overlaps(networks[i].Status.CIDRBlocks, networks[j].Status.CIDRBlocks) {
				overlaps = append(overlaps, networkRef(&networks[j]))
			}
		}
		slices.Sort(overlaps)
		if equality.Semantic.DeepEqual(networks[i].Status.OverlapsWith, overlaps) {
			continue
		}
		networks[i].Status.OverlapsWith = overlaps
		if err := r.Status().Update(ctx, &networks[i]); err != nil {
			return nil, err
		}
	}
	return networks, nil
...
func (r *NetworkScopeReconciler) updateStatus(ctx context.Context, scope *networkv1beta1.NetworkScope,
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		unmanaged := int32(0)
		for _, t := range targets {
			unmanaged += t.UnmanagedNetworks + t.UnmanagedSubnets
		}
