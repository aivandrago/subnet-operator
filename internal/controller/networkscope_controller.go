...
type NetworkScopeReconciler struct {
	...
	// Concurrency is the number of account/region pairs discovered in parallel.
	Concurrency int
...
func (r *NetworkScopeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	if !full {
		...
		if len(targets) == 0 {
			return ctrl.Result{RequeueAfter: untilFullSync(scope, now)}, nil
		}
	...
	statusTargets, err := r.updateStatus(ctx, scope, all, results, full, len(vpcs), len(subnets.Items), syncTime)
	if err != nil {
		return ctrl.Result{}, err
	}
	metrics.SetScope(scope.Name, vpcs, subnets.Items, metricTargets(statusTargets, results), float64(now.Unix()))

	if full {
		return ctrl.Result{RequeueAfter: resyncInterval(scope)}, nil
	}
	return ctrl.Result{RequeueAfter: untilFullSync(scope, now)}, nil
}
...
func metricTargets(targets []awsv1alpha1.TargetStatus, results []targetResult) []metrics.TargetResult {
	...
	for _, t := range targets {
		out = append(out, metrics.TargetResult{
			Account: t.Account, Region: t.Region, OK: t.Error == "",
			Synced: synced[inventory.TargetKey{Account: t.Account, Region: t.Region}],
		})
	}
	return out
}
...
) ([]awsv1alpha1.TargetStatus, error) {
