...
var _ = Describe("SubnetClaim Controller", func() {
	...
	getClaim := func() *awsv1alpha1.SubnetClaim {
		GinkgoHelper()
		c := &awsv1alpha1.SubnetClaim{}
		Expect(k8sClient.Get(ctx, types.NamespacedName{Name: claimName, Namespace: "default"}, c)).To(Succeed())
		return c
	}
	createClaim := func(mutate func(*awsv1alpha1.SubnetClaim)) {
		GinkgoHelper()
		c := &awsv1alpha1.SubnetClaim{
			ObjectMeta: metav1.ObjectMeta{Name: claimName, Namespace: "default"},
			Spec: awsv1alpha1.SubnetClaimSpec{
				ScopeRef: scopeName, Account: claimAccount, Region: claimRegion, VPCID: claimVPC,
				PrefixLength: 24, AvailabilityZones: []string{claimRegion + "a", claimRegion + "b"},
				Mode: awsv1alpha1.ClaimModeCreate, Owner: "team-payments", Env: "prod", Tier: "private",
	...
	createSubnet := func(id, cidr, az string, tags map[string]string) {
		...
		sn := &awsv1alpha1.Subnet{
			...
			Spec:       awsv1alpha1.SubnetSpec{SubnetID: id, VPCID: claimVPC, Account: claimAccount, Region: claimRegion},
		}
		Expect(k8sClient.Create(ctx, sn)).To(Succeed())
		sn.Status = awsv1alpha1.SubnetStatus{CIDRBlock: cidr, AvailabilityZone: az, Tags: tags}
		Expect(k8sClient.Status().Update(ctx, sn)).To(Succeed())
	}
	cond := func(c *awsv1alpha1.SubnetClaim, typ string) *metav1.Condition {
	...
	BeforeEach(func() {
		...
		labels = map[string]string{awsv1alpha1.LabelScope: scopeName, awsv1alpha1.LabelAccount: claimAccount,
			awsv1alpha1.LabelRegion: claimRegion, awsv1alpha1.LabelVPC: claimVPC}
		...
		vpc := &awsv1alpha1.VPC{
			...
			Spec:       awsv1alpha1.VPCSpec{VPCID: claimVPC, Account: claimAccount, Region: claimRegion},
		}
	...
	It("reserves free CIDRs and creates tagged subnets", func() {
		...
		Expect(c.Status.Allocations[0].AvailabilityZone).To(Equal(claimRegion + "a"))
		...
		Expect(c.Status.Allocations[0].State).To(Equal(awsv1alpha1.AllocationCreated))
		...
		Expect(req.Tags).To(HaveKeyWithValue(awsv1alpha1.TagManagedBy, awsv1alpha1.TagManagedByValue))
		Expect(req.Tags).To(HaveKeyWithValue(awsv1alpha1.TagClaim, "default/"+claimName))
	...
	It("only reserves CIDRs in Allocate mode", func() {
		createClaim(func(c *awsv1alpha1.SubnetClaim) { c.Spec.Mode = awsv1alpha1.ClaimModeAllocate })
		Expect(reconcileClaim()).To(Succeed())
		c := getClaim()
		Expect(cond(c, ConditionReady).Status).To(Equal(metav1.ConditionTrue))
		Expect(cond(c, ConditionReady).Reason).To(Equal("Allocated"))
		Expect(c.Status.Allocations[0].State).To(Equal(awsv1alpha1.AllocationPending))
	...
	It("takes reservations of other claims into account", func() {
		other := &awsv1alpha1.SubnetClaim{
			ObjectMeta: metav1.ObjectMeta{Name: claimName + "-other", Namespace: "default"},
			Spec: awsv1alpha1.SubnetClaimSpec{ScopeRef: scopeName, Account: claimAccount, Region: claimRegion,
				VPCID: claimVPC, PrefixLength: 24, AvailabilityZones: []string{claimRegion + "a"}, Owner: "x"},
		}
		Expect(k8sClient.Create(ctx, other)).To(Succeed())
		other.Status.Allocations = []awsv1alpha1.SubnetAllocation{{AvailabilityZone: claimRegion + "a", CIDRBlock: "10.50.2.0/24", State: awsv1alpha1.AllocationPending}}
	...
	It("keeps the reservation when creation fails for another reason", func() {
		...
		c := getClaim()
		Expect(cond(c, ConditionReady).Reason).To(Equal("CreateFailed"))
		Expect(c.Status.Allocations[0].State).To(Equal(awsv1alpha1.AllocationFailed))
	...
	It("adopts subnets that already carry the claim tag", func() {
		createSubnet("subnet-adopt", "10.50.9.0/24", claimRegion+"a",
			map[string]string{awsv1alpha1.TagClaim: "default/" + claimName})
		...
		Expect(a.State).To(Equal(awsv1alpha1.AllocationCreated))
	...
	It("reports a VPC that was not discovered", func() {
		createClaim(func(c *awsv1alpha1.SubnetClaim) { c.Spec.VPCID = "vpc-0deadbeef0" })
		Expect(reconcileClaim()).To(Succeed())
		Expect(cond(getClaim(), ConditionReady).Reason).To(Equal("VPCNotFound"))
		Expect(writer.requests).To(BeEmpty())
	})

	It("allocates but does not create for a spoke without a write role", func() {
		spokeVPC := fmt.Sprintf("vpc-5b0ce%04d", claimCounter)
		spokeLabels := map[string]string{awsv1alpha1.LabelScope: scopeName, awsv1alpha1.LabelAccount: spokeAccount,
			awsv1alpha1.LabelRegion: claimRegion, awsv1alpha1.LabelVPC: spokeVPC}
		vpc := &awsv1alpha1.VPC{
			ObjectMeta: metav1.ObjectMeta{Name: spokeVPC, Labels: spokeLabels},
			Spec:       awsv1alpha1.VPCSpec{VPCID: spokeVPC, Account: spokeAccount, Region: claimRegion},
		}
		Expect(k8sClient.Create(ctx, vpc)).To(Succeed())
		vpc.Status.CIDRBlocks = []string{"10.60.0.0/16"}
		Expect(k8sClient.Status().Update(ctx, vpc)).To(Succeed())

		createClaim(func(c *awsv1alpha1.SubnetClaim) { c.Spec.Account = spokeAccount; c.Spec.VPCID = spokeVPC })
		Expect(reconcileClaim()).To(Succeed())
		c := getClaim()
		Expect(cond(c, ConditionAllocated).Status).To(Equal(metav1.ConditionTrue))
