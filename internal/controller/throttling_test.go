...
var _ = Describe("NetworkScope Controller under API throttling", func() {
	...
	BeforeEach(func() {
		...
		Expect(k8sClient.Create(ctx, &awsv1alpha1.NetworkScope{
			...
			Spec: awsv1alpha1.NetworkScopeSpec{
				Accounts:       []awsv1alpha1.AccountSpec{{ID: accountA}, {ID: accountB}},
	...
	It("keeps a throttled target's inventory, backs it off on its own schedule and recovers on its own", func() {
		...
		Expect(b.VPCs).To(Equal(int32(1)), "the last known numbers stay")
...
var _ = Describe("NetworkScope discovery concurrency", func() {
	It("caps discoveries across scopes, not per scope", func() {
		...
		for i := range 3 {
			...
			accounts := make([]awsv1alpha1.AccountSpec, 0, 4)
			for j := range 4 {
				accounts = append(accounts, awsv1alpha1.AccountSpec{ID: fmt.Sprintf("3%02d%09d", i, j)})
			}
			scope := &awsv1alpha1.NetworkScope{
				ObjectMeta: metav1.ObjectMeta{Name: name},
				Spec:       awsv1alpha1.NetworkScopeSpec{Accounts: accounts, Regions: []string{region}},
			}
