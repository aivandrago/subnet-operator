...
func (v *SubnetClaimValidator) poolFor(ctx context.Context, claim *networkv1.SubnetClaim,
	network *networkv1.Network) (allocator.Pool, error) {
	pool := allocator.Pool{CIDRs: network.Status.CIDRBlocks}

	subnets := &networkv1.SubnetList{}
	if err := v.Client.List(ctx, subnets, client.MatchingLabels{networkv1.LabelNetwork: claim.Spec.NetworkID}); err != nil {
