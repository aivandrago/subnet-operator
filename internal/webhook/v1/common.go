...
// Package v1 holds the webhooks for the network.hypersurgery.dev API group: admission for v1,
// and the conversion between v1 and v1beta1.
//
// Admission is registered for v1 only. The API server converts a request made at v1beta1 to
// v1 before it calls an admission webhook (matchPolicy Equivalent, the default), so every
// served version gets the same checks.
//
...
func invalidError(kind, name string, errs field.ErrorList) error {
	...
	gk := schema.GroupKind{Group: networkv1.SchemeGroupVersion.Group, Kind: kind}
...
// done without it.
func providerFor(providers *provider.Registry, scope *networkv1.NetworkScope, scopeRef string) (
...
func scopeFor(ctx context.Context, reader client.Reader, path *field.Path, name string) (
	*networkv1.NetworkScope, *field.Error) {
...
func validateNamespace(ctx context.Context, reader client.Reader, scope *networkv1.NetworkScope,
...
func validateCreatedByOnCreate(ctx context.Context, obj client.Object) *field.Error {
	...
	got := obj.GetAnnotations()[networkv1.AnnotationCreatedBy]
...
func validateCreatedByOnUpdate(oldObj, newObj client.Object) *field.Error {
	oldValue, hadIt := oldObj.GetAnnotations()[networkv1.AnnotationCreatedBy]
	newValue, hasIt := newObj.GetAnnotations()[networkv1.AnnotationCreatedBy]
...
func createdByPath() *field.Path {
	return field.NewPath("metadata").Child("annotations").Key(networkv1.AnnotationCreatedBy)
}
...
func stampCreatedBy(ctx context.Context, obj client.Object) {
	...
	annotations[networkv1.AnnotationCreatedBy] = req.UserInfo.Username
