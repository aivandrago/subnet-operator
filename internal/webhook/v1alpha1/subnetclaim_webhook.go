...
// Default writes the name prefix into the spec. The controller already falls back to the
// claim's name; doing it here as well means the Name tag the subnets will carry is visible
// in the object instead of only in the operator's head.
func (d *SubnetClaimDefaulter) Default(_ context.Context, obj *awsv1alpha1.SubnetClaim) error {
