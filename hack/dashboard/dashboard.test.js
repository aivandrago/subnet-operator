...
function hostileData() {
  return {
    networkscopes: { items: [{
      ...
      spec: { networkSelector: { matchTags: { 'team/inventory': 'yes' } }, tagKeys: { owner: 'team', env: 'stage', tier: 'layer' } },
      status: { targets: [
        { account: '111111111111', region: 'eu-central-1' + EVIL, subnets: 3, networks: 1, lastSyncTime: '2026-09-24T10:00:00Z',
          unmanagedIDs: ['vpc-0aaa' + EVIL, 'subnet-0bbb'] },
        { account: '222222222222', region: 'eu-west-1', error: 'AccessDenied' + EVIL },
      ] },
    }] },
    networks: { items: [{ status: { overlapsWith: ['x'] } }, { status: {} }] },
    subnets: { items: [
      { spec: { id: 'subnet-01' + EVIL, account: '111111111111', region: 'eu-central-1', networkID: 'vpc-1' },
        status: { name: EVIL, owner: EVIL, env: 'prod', totalIPs: 100, availableIPs: 10, missingTags: [EVIL] } },
      { spec: { id: 'subnet-02' }, status: { env: 'prod', totalIPs: 100, availableIPs: 90 } },
      { spec: { id: 'subnet-03' }, status: { totalIPs: 0, availableIPs: 0 } },
      { spec: { id: 'subnet-04' }, status: { env: EVIL, totalIPs: 0, availableIPs: 0 } },
    ] },
    subnetclaims: { items: [{
      metadata: { namespace: 'payments', name: 'checkout' + EVIL },
      spec: { networkID: 'vpc-1', prefixLength: 24, owner: EVIL },
      status: { allocations: [{ name: 'checkout-a', zone: 'eu-central-1a', cidrBlock: EVIL, state: 'Failed', error: EVIL }],
...
test('a subnet whose free addresses are unknown is not counted as full', () => {
  const { live } = load();
  const data = hostileData();
  // A provider that cannot count the free addresses leaves availableIPs out.
  data.subnets.items.push({ spec: { id: 'subnet-05' }, status: { env: 'prod', totalIPs: 100 } });
  const v = live.summarize(data);
  assert.equal(v.tiles.hot, 1, 'still only the subnet at 90%');
  assert.equal(v.envs.find((e) => e.env === 'prod').used, 0.5, 'nothing added to prod');
});
...
test('a watched kind is listed once, then kept current by its watch', async () => {
  ...
  assert.deepEqual([...sync.needed()], ['networkscopes', 'networks', 'subnets', 'subnetclaims', 'resourceimports']);
  ...
  assert.deepEqual([...sync.needed()], ['networkscopes', 'networks', 'subnetclaims', 'resourceimports'], 'a watched kind is not polled');
...
test('without a way to stream, every watched kind falls back to polling', async () => {
  ...
  await sync.load(sync.needed());
  await timers.run();
  assert.deepEqual([...sync.needed()], ['networkscopes', 'networks', 'subnetclaims']);
...
test('an import shows the authenticated creator, and requestedBy only as who it was for', () => {
  ...
  data.resourceimports.items = [
    { metadata: { namespace: 'default', name: 'forged', creationTimestamp: '2026-09-24T09:00:00Z',
      annotations: { 'network.hypersurgery.dev/created-by': 'jane@example.com' } },
