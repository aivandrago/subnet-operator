...
// +kubebuilder:deprecatedversion:warning="aws.hypersurgery/v1alpha1 NetworkScope is deprecated and removed in 0.9; the operator migrates it to network.hypersurgery.dev/v1beta1 NetworkScope, use that instead"
...
// the scope. An unset selector allows every namespace, for compatibility with the releases
// before it existed; a selector that does not parse allows none, because a typo in the one
// field that limits who may use the write roles must not open them to everybody.
