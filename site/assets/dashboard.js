...
(function (global) {
  ...
  function summarize(data) {
    ...
    var networks = arr(obj(data.networks).items);
    ...
    var subnets = subnetItems.map(function (s) {
      ...
      return {
        id: str(spec.id) || str(obj(s.metadata).name), name: str(st.name),
    ...
    var claims = arr(obj(data.subnetclaims).items).map(function (c) {
      ...
      return {
        ns: str(md.namespace), name: str(md.name), vpc: str(spec.networkID), prefix: num(spec.prefixLength),
        ...
        allocations: arr(obj(c.status).allocations).map(function (a) {
          a = obj(a);
          return { az: str(a.zone), cidr: str(a.cidrBlock), subnet: str(a.subnetID), state: str(a.state) || 'Pending', error: str(a.error) };
        })
      };
    });
  ...
  function importObject(resource, form, scope) {
    ...
    if (resource.kind === 'vpc') {
      // Discovery picks a VPC up by the scope's network selector, so those are the tags that
      // make it managed. An empty selector value matches any value; "true" is as good as any.
