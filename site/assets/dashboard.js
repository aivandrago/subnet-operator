...
(function (global) {
  ...
  function apiClient(source, fetchImpl) {
    ...
    function request(method, path, body) {
      ...
      var init = { method: method, headers: headers, cache: 'no-store', redirect: 'error' };
      ...
      return fetchImpl(source.base + path, init).then(function (res) {
        return res.text().then(function (text) {
          var data = null;
          try { data = text ? JSON.parse(text) : null; } catch (e) { data = null; }
          if (!res.ok) {
            var err = new Error((data && typeof data.message === 'string' && data.message) || ('HTTP ' + res.status));
            err.code = res.status;
            throw err;
          }
          return data;
  ...
  // loadAll lists every resource on its own, so one the viewer may not read leaves only its
  // own panel empty instead of the whole page.
  function loadAll(api) {
    return Promise.all(LIVE_RESOURCES.map(function (r) {
      return api.list(r).then(
        function (d) { return { items: (d && Array.isArray(d.items)) ? d.items : [] }; },
        function (e) { return { items: [], error: e }; }
      );
    })).then(function (results) {
      var data = {};
      LIVE_RESOURCES.forEach(function (r, i) { data[r] = results[i]; });
      return data;
    });
  }
    ...
    '<div class="dash-body">' +
      ...
      '<div class="dash-grid">' +
        ...
        '<div class="dash-card">' +
          ...
          '<p class="sub">ResourceImports, newest first, and who asked for them.</p>' +
          '<table><thead><tr><th class="mono">Resource</th><th class="hide-sm">Requested by</th><th>State</th></tr></thead>' +
    ...
    fill(el.imports, data.resourceimports.error ? [deniedRow(3, data.resourceimports.error)]
      : view.imports.length ? view.imports.map(function (i) {
        ...
        return h('tr', {}, [
          ...
          h('td', { 'class': 'hide-sm' }, [i.requestedBy]),
  ...
  function renderLiveDrawer(host, draft) {
    ...
    fill(host, [h('form', { 'class': 'dash-drawer', 'data-el': 'importForm' }, [
      ...
      h('div', {}, [
        ...
        h('p', { 'class': 'sub dash-gap-top' }, ['The API server records you as the creator, and the operator fills in requestedBy with your user name. ' +
  ...
  function mountLive(container, options) {
    ...
    var data = null, view = null, loadedAt = 0, timer = null, draft = null, loading = false;
    ...
    function refresh() {
      ...
      loading = true;
      return loadAll(api).then(function (d) {
        loading = false;
        var errors = LIVE_RESOURCES.map(function (r) { return d[r].error; }).filter(Boolean);
        var unauthorized = errors.length === LIVE_RESOURCES.length && errors.every(function (e) { return e.code === 401; });
        ...
        el.auth.hidden = true;
        if (errors.length === LIVE_RESOURCES.length) {
        ...
        showBanner('');
        data = d; view = summarize(d); loadedAt = Date.now();
        redraw();
        tickSync();
      });
    }
    ...
    function tickSync() {
      ...
      el.sync.textContent = s < 2 ? 'loaded just now' : 'loaded ' + s + 's ago';
    }
    ...
    el.signout.addEventListener('click', function () {
      writeToken('');
      data = null; view = null; loadedAt = 0;
      refresh();
    });
    ...
    function start() {
      ...
      timer = setInterval(function () {
        ...
        tickSync();
        if (Date.now() - loadedAt >= REFRESH_MS && !draft) refresh();
      }, 1000);
    }
    function stop() { clearInterval(timer); timer = null; }
