...
**Alpha, v0.6.0.** Read-only discovery is the default and the part that has had the most use:
...
organization — that conformance run is the open item in the current milestone.
...
per target, a full sync of 400 targets takes about 150 s: a quarter of the interval, which
leaves the rest as headroom for accounts that EC2 throttles, whose calls are retried and can
take a minute instead of a second.
...
The arithmetic, the API cost per sync and the memory side are in
