...
**Alpha, v0.7.1.** This is the last release under the name aws-subnet-operator; the project continues as subnet-operator from v0.8.0. Read-only discovery is the default and the part that has had the most use:
...
Next, in 0.8: the API moves to the cloud-neutral group `network.hypersurgery.dev` and the
project is renamed to `subnet-operator`; the operator migrates existing objects itself. See
...
- **In the cluster**: `dashboard.enabled=true` in the chart runs it as a second entrypoint of the
  ...
  from your SSO proxy, or pasted into the app — for reads of the `aws.hypersurgery` group and
  the creation of `ResourceImport`s, and nothing else. See the
  [chart README](charts/aws-subnet-operator/README.md#the-dashboard-app).
...
- **Only the namespaces a scope allows.** `spec.namespaceSelector` on the `NetworkScope` names
  ...
  Without a selector every namespace may (the v1alpha1 default, warned about). Who should hold
  `create` on claims and imports is in the
...
`aws.hypersurgery/created-by` annotation names the Kubernetes identity that created the
...
| 5. Cloud-neutral API (0.8) | `network.hypersurgery.dev` with the provider as a field, operator-driven migration from `aws.hypersurgery`, the project renamed to `subnet-operator` ([ADR 0002](docs/adr/0002-multi-cloud-model.md)) | One API for every cloud |
| 6. 1.0 | `v1` API with a compatibility promise, where a scope without a namespace selector allows no namespace, the [support policy](docs/policy.md) in full | Stable |
...
- A scope's write roles can be limited to the namespaces its `namespaceSelector` selects.
