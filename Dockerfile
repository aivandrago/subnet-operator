# Build the manager and dashboard binaries
...
# The dashboard ships in the same image as a second entrypoint, so the image's signature and
# SBOM cover it too. The Helm chart starts it with command: ["/dashboard"].
